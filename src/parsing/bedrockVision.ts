...
- "items": una entrada por cada línea de PRODUCTO de la factura (no de la sección de impuestos). Tomá "quantity" de la columna Cantidad y "unitPrice" de la columna Precio de esa fila. "subtotal" = el valor de la columna Total/Importe de ESA MISMA fila; si esa columna no está o no se lee, usá quantity × unitPrice. Siempre tiene que cumplirse subtotal = quantity × unitPrice para cada ítem — si no da exacto, algo se leyó mal, revisá los tres valores de esa fila. "voided" true solo si la línea figura anulada.
...
- Si la factura tiene una sección de impuestos ("Impuestos", "IVA") con columnas Base e Impuesto: poné en "tax" el total de la columna Impuesto. La Base de esa sección es un total agregado de TODA la factura, nunca el "subtotal" de un ítem puntual — no la uses ahí ni aunque coincida con un ítem. Usala como "subtotal" de un ítem SOLO en el caso excepcional de que no puedas leer NINGÚN precio por línea (columna Precio/Total cortada o ilegible en todos los ítems); ahí sí, la suma de los "subtotal" de los ítems debe dar la Base, y "total" = Base + Impuesto = "VALOR A PAGAR".
